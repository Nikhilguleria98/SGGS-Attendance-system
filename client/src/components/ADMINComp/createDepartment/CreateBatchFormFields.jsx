...
const CreateBatchFormFields = ({ onSubmitSuccess, initialData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = initialData ? `${import.meta.env.VITE_API_URL}/batches/${initialData._id}` : `${import.meta.env.VITE_API_URL}/batches`;
      ...
      console.error(err);
      setIsLoading(false);
      setErrors({ form: err.message || "Failed to create batch. Please try again." });
    }
  };
      ...
      <AnimatePresence>
        {isSuccess && (
          ...
          >
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-1">{initialData ? "Batch Updated!" : "Batch Created!"}</h3>
            <p className="text-gray-500 text-sm">
              {initialData ? "The batch has been updated successfully." : "The batch has been added successfully."}
            </p>
          ...
          >
            {isLoading ? (
              <>
                ...
                <span>{initialData ? "Updating Batch..." : "Creating Batch..."}</span>
              </>
            ) : (
              <span>{initialData ? "Update Batch" : "Create Batch"}</span>
            )}
          </motion.button>
        </form>
      )}
...
export default CreateBatchFormFields;
