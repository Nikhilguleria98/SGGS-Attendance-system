    ...
    <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-8 text-center">
          <p className="text-gray-500 font-medium">No batches match your search.</p>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Batch Name</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.map((batch) => (
                <tr key={batch._id} className="hover:bg-blue-50/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-500 max-w-md truncate">{batch.description || "N/A"}</div>
                  </td>
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-3">
                      {onEdit && (
                        <button onClick={() => onEdit(batch)} className="text-gray-400 hover:text-[#00529b] transition-colors" title="Edit Batch">
                          <Edit2 size={18} />
                        </button>
                      )}
                      {onDelete && (
                        <button onClick={() => onDelete(batch)} className="text-gray-400 hover:text-red-600 transition-colors" title="Delete Batch">
                          <Trash2 size={18} />
                        </button>
                      )}
                    </div>
